...
func (h *UIHandler) renderAdminPage(w http.ResponseWriter, r *http.Request, activePage, pageTitle, contentTemplate string) {
	h.renderAdminPageWithData(w, r, activePage, pageTitle, contentTemplate, nil)
}

// renderAdminPageWithData renders a page with the admin layout and passes data to the content template
func (h *UIHandler) renderAdminPageWithData(w http.ResponseWriter, r *http.Request, activePage, pageTitle, contentTemplate string, contentData interface{}) {
	...
	// Load user info
	user, err := h.users.GetByID(tenantID, userID)
	if err != nil {
	...
	// Load tenant info
	tenant, err := h.tenants.Get(tenantID)
	if err != nil {
...
func (h *UIHandler) AdminDashboardHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.EqualFold(tenantID, h.config.GetSystemTenantID()) {
			if tenantCount, err = h.tenants.Count(); err != nil {
		...
		userCount := 0
		if userCount, err = h.users.Count(tenantID); err != nil {
		...
		if sessionCount, err = h.refreshTokens.Count(tenantID); err != nil {
		...
		clientCount := 0
		if clientCount, err = h.clients.Count(tenantID); err != nil {
...
func (h *UIHandler) AdminTenantsListHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		// Fetch all tenants
		tenantsResponse, err := h.tenants.List(0, 100)
		if err != nil {
		...
		h.renderAdminPageWithData(w, r, "tenants", "Tenants", "admin_tenants_content.html", data)
	}
}

// AdminTenantNewHandler shows the form to create a new tenant
func (h *UIHandler) AdminTenantNewHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
...
func (h *UIHandler) AdminTenantEditHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		// Fetch the tenant
		tenant, err := h.tenants.Get(editTenantID)
		if err != nil {
			http.Error(w, "Tenant not found", http.StatusNotFound)
			return
		}
		...
		accessTokenExpiry := tenant.Config.GetAccessTokenExpiry(h.config.GetDefaultAccessTokenExpiry())
		idTokenExpiry := tenant.Config.GetIDTokenExpiry(h.config.GetDefaultIDTokenExpiry())
		refreshTokenExpiry := tenant.Config.GetRefreshTokenExpiry(h.config.GetDefaultRefreshTokenExpiry())
		...
		h.renderAdminPageWithData(w, r, "tenants", "Edit Tenant", "admin_tenant_form.html", data)
	}
}

// AdminTenantSaveHandler handles create/update of tenants
func (h *UIHandler) AdminTenantSaveHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		if isNew {
			...
			config := tenants.TenantConfig{
				Issuer:             issuer,
				Audience:           audience,
				LoginURL:           strings.TrimRight(issuer, "/") + RouteAuthLogin,
			...
			if err := h.tenants.Upsert(tenant); err != nil {
				w.Header().Set("Content-Type", "text/html")
				w.WriteHeader(http.StatusInternalServerError)
			...
			tenant, err := h.tenants.Get(newTenantID)
			if err != nil {
				w.Header().Set("Content-Type", "text/html")
			...
			tenant.Config.LoginURL = strings.TrimRight(issuer, "/") + RouteAuthLogin
...
func (h *UIHandler) AdminClientsListHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		clientsList, err := h.clients.List(tenantID, 0, 100)
		if err != nil {
...
func (h *UIHandler) AdminClientNewHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		data := map[string]interface{}{
			"IsNew": true,
		}
		h.renderAdminPageWithData(w, r, "clients", "Create Client", "admin_client_form.html", data)
	}
}

// AdminClientCheckDuplicateHandler checks if a client ID already exists
func (h *UIHandler) AdminClientCheckDuplicateHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		existingClient, err := h.clients.Get(tenantID, clientID)
...
func (h *UIHandler) AdminClientEditHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		// Fetch client
		client, err := h.clients.Get(tenantID, editClientID)
		if err != nil {
		...
		h.renderAdminPageWithData(w, r, "clients", "Edit Client", "admin_client_form.html", data)
	}
}

// AdminClientSaveHandler handles create/update of clients
func (h *UIHandler) AdminClientSaveHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		if isNew {
			existingClient, _ := h.clients.Get(tenantID, clientID)
		...
		if err := h.clients.Upsert(tenantID, client); err != nil {
			w.Header().Set("Content-Type", "text/html")
			w.WriteHeader(http.StatusInternalServerError)
...
func (h *UIHandler) AdminClientValidateIDHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if err := r.ParseForm(); err != nil {
			http.Error(w, "Failed to parse form", http.StatusBadRequest)
			return
...
func (h *UIHandler) AdminTenantDeleteHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		if strings.EqualFold(deleteTenantID, h.config.GetSystemTenantID()) {
			w.Header().Set("Content-Type", "text/html")
			w.WriteHeader(http.StatusForbidden)
		...
		// Delete the tenant
		if err := h.tenants.Delete(deleteTenantID); err != nil {
			w.Header().Set("Content-Type", "text/html")
			w.WriteHeader(http.StatusInternalServerError)
...
func (h *UIHandler) AdminClientDeleteHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Get tenant ID from context
		tenantID, _ := r.Context().Value(ContextKeyTenantID).(string)
		...
		// Delete the client
		if err := h.clients.Delete(tenantID, clientID); err != nil {
			w.Header().Set("Content-Type", "text/html")
			w.WriteHeader(http.StatusInternalServerError)
...
func (h *UIHandler) AdminUsersListHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		h.renderAdminPage(w, r, "users", "Users", "admin_users_content.html")
	}
}

// AdminSettingsHandler shows settings page
func (h *UIHandler) AdminSettingsHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		h.renderAdminPage(w, r, "settings", "Settings", "admin_settings_content.html")
	}
}

// AdminProfileHandler shows user profile page
func (h *UIHandler) AdminProfileHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		h.renderAdminPage(w, r, "", "Profile", "admin_profile_content.html")
	}
}
